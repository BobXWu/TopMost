...
def dynamic_model_test(model, dataset, num_topics):

    trainer = CrosslingualTrainer(model, dataset)

    beta_en, beta_cn = trainer.export_beta()
    assert beta_en.shape == beta_cn.shape
    assert beta_en.shape == (num_topics, dataset.vocab_size)

    train_theta_en, train_theta_cn, test_theta_en, test_theta_cn = trainer.export_theta()
    assert train_theta_en.shape == (len(dataset.train_texts), num_topics)
    assert test_theta_en.shape == (len(dataset.test_texts), num_topics)
    assert train_theta_cn.shape == (len(dataset.train_texts), num_topics)
    assert test_theta_cn.shape == (len(dataset.test_texts), num_topics)
...
def test_models(cache_path):
    ...
    dataset = CrosslingualDatasetHandler(f"{cache_path}/Amazon_Review", lang1='en', lang2='cn', dict_path=f'{cache_path}/ch_en_dict.dat', as_tensor=True)
    ...
    dynamic_model_test(model, dataset, num_topics)
